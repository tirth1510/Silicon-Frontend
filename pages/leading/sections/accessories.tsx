...
function FeaturedAccessoriesContent({
  preFetchedAccessories,
}: {
  preFetchedAccessories?: any;
}) {
  ...
  useEffect(() => {
    if (preFetchedAccessories) {
      const rawData =
        preFetchedAccessories.products || preFetchedAccessories.data || [];
      const mapped: ProductForList[] = rawData.map(
  ...
  const handleSubmitEnquiry = () => {
    ...
    // 2️⃣ Prepare payload - ✅ ADDED productId & productImageUrl
    const payload = {
      name: form.name,
      email: form.email,
      phone: form.phone,
      message: form.message,
      productTitle: selectedProduct?.productname || "Medical Accessory",
      productId: selectedProduct?.id || "unknown",
      productImageUrl: selectedProduct?.productImage || FALLBACK_IMAGE,
    };
...
export default function FeaturedAccessories(props: {
  preFetchedAccessories?: any;
}) {
  return (
    <Providers>
      <FeaturedAccessoriesContent {...props} />
    </Providers>
  );
}
