...
export default function AccessoryDetailsPage() {
  ...
  const accessoryImages = React.useMemo(() => {
    ...
    const images: string[] = [];
    const productImgs =
      accessory.productImages || accessory.productImageUrl || [];
    productImgs.forEach((img: any) => img.url && images.push(img.url));
    const galleryImgs =
      accessory.galleryImages || accessory.productGallery || [];
    galleryImgs.forEach(
      (img: any) =>
        img.url && !images.includes(img.url) && images.push(img.url),
    );
  ...
  const handleSubmitEnquiry = () => {
    ...
    const payload = {
      ...
      productTitle: accessory?.productTitle || "Medical Accessory",
      productId: accessory?._id || accessory?.id || "unknown",
      productImageUrl: accessory?.productImageUrl?.[0]?.url || FALLBACK_IMAGE,
    };
          ...
          <div className="lg:col-span-6 space-y-6 sm:space-y-8">
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
                ...
                <span className="bg-green-100 text-green-700 px-2 py-1 rounded italic">
              ...
              <div className="flex items-center gap-4 text-xs sm:text-sm font-medium">
                ...
                <span className="text-slate-500 underline underline-offset-4 cursor-pointer text-xs">
                  Official Accessory
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              <div className="p-3 sm:p-4 rounded-xl border border-slate-100 bg-slate-50 flex items-center gap-3 sm:gap-4">
                <div className="bg-white p-2 sm:p-2.5 rounded-lg text-blue-900 shadow-sm">
                  <ShieldCheck className="w-5 h-5 sm:w-[22px] sm:h-[22px]" />
                </div>
                <div>
                  <h4 className="text-xs sm:text-sm font-bold text-slate-900">
                    Warranty
                  </h4>
                  <p className="text-[10px] sm:text-[11px] text-slate-500 font-medium tracking-tight">
                    Standard Support
                  </p>
                </div>
              </div>
              ...
              accessory.specifications.length > 0 && (
                <div className="pt-2 space-y-3 sm:space-y-4">
                  <h3 className="text-base sm:text-lg font-bold text-slate-900 flex items-center gap-2">
                    <Award
                      size={18}
                      className="text-blue-900 sm:w-[20px] sm:h-[20px]"
                    />{" "}
                    Key Features
                  </h3>
                  <div className="space-y-2 sm:space-y-3">
                    {accessory.specifications.map((spec: any, i: number) => (
                      <div
                        key={i}
                        className="flex items-start gap-2 sm:gap-3 p-2 rounded-xl hover:bg-slate-50 transition-all"
                      >
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {accessory.productSpecifications.map(
                      (f: any, i: number) => (
                        <div
                          key={i}
                          className="flex justify-between items-center p-3 sm:p-4 bg-white border border-slate-100 rounded-xl"
                        >
                          <span className="text-[9px] sm:text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">
                            {f.key}
                          </span>
                          <span className="text-xs sm:text-sm font-bold text-slate-900 text-right ml-4">
                            {f.value}
                          </span>
                        </div>
                      ),
          ...
          {accessory && (
            <div className="space-y-4 mt-2">
              <div className="flex items-center gap-3 sm:gap-4 bg-blue-50 p-3 rounded-2xl border border-blue-100">
                ...
                <div>
                  <p className="text-[9px] sm:text-[10px] font-bold text-blue-600 uppercase">
                    Selected Accessory
                  </p>
                  <h4 className="font-bold text-sm sm:text-base text-blue-900 leading-tight line-clamp-2">
                    {accessory.productTitle}
                  </h4>
                </div>
              </div>
