...
export const accessoriesColumns = [
  ...
  },
  {
    key: "discount",
    label: "Discount",
    render: (row: Product) => (row.discount ? `${row.discount}%` : "-"),
  },
  {
    key: "finalPrice",
    label: "Final Price",
    render: (row: Product) => (row.finalPrice ? `₹${row.finalPrice}` : "-"),
  },
  {
    key: "stock",
    label: "Stock",
    ...
    render: (row: Product) => (
      <div className="flex gap-2">
        ...
        <button
          ...
          onClick={async () => {
            try {
              await updateProductStatusService(row.id, "Enquiry");
